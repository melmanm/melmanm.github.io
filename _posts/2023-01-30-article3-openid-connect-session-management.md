...
* **Enabling Single sign-on (SSO)** - SSO enables reusing the same user session for across multiple applications launched in the same web browser and working under the same Identity Provider Server. Once user signs-is into single application, other applications from the same suite or family can be used without user being ask to sign-in again. 
